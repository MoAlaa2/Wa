  ...
  return (
    <div className={`fixed inset-0 z-50 overflow-hidden ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
      ...
      <div className={`fixed inset-y-0 ${dir === 'rtl' ? 'left-0' : 'right-0'} max-w-full flex`}>
        <div className={`w-screen max-w-lg transform transition ease-in-out duration-300 ${isOpen ? 'translate-x-0' : (dir === 'rtl' ? '-translate-x-full' : 'translate-x-full')} bg-white shadow-xl flex flex-col`}>
          
          {/* Header */}
          <div className="px-6 py-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
            <div>
              <h2 className="text-lg font-bold text-gray-900">{t.orders.table.orderNum} #{order.orderNumber}</h2>
              <span className={`text-xs px-2 py-0.5 rounded font-medium ${order.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                {t.orders.status[order.status === 'completed' ? 'paid' : order.status === 'pending-payment' ? 'pendingPayment' : 'processing']}
              </span>
            </div>
            <div className="flex items-center gap-2">
              {isAdmin && !isEditing && (
                <button onClick={startEdit} className="p-2 text-gray-500 hover:bg-gray-100 rounded-full" title={t.common.edit}>
                  <Edit2 size={18} />
                </button>
              )}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            {order.approvalStatus === 'pending_approval' && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-bold text-yellow-800">{t.orders.drawer.approvalRequired}</h3>
                  <p className="text-sm text-yellow-700 mt-1">{t.orders.drawer.approvalDesc}</p>
                </div>
              </div>
            )}
            ...
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-gray-400 uppercase">{t.orders.table.customer}</h4>
              <div className="bg-gray-50 p-3 rounded-lg border border-gray-100 flex justify-between items-center">
                ...
                >
                  <MessageSquare size={16}/> {t.orders.drawer.openChat}
                </button>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-gray-400 uppercase flex items-center gap-1">
                <Truck size={12}/> {t.orders.drawer.fulfillment}
              </h4>
              <div className="border border-gray-200 rounded-lg p-3">
                {isEditing ? (
                  <div className="space-y-3">
                    ...
                    >
                      <option value="delivery">{t.orders.drawer.delivery}</option>
                      <option value="pickup">{t.orders.drawer.pickup}</option>
                      <option value="none">{t.orders.drawer.noShipping}</option>
                    </select>
                    {editForm.fulfillment?.type === 'pickup' && (
                      <input 
                        placeholder={t.orders.drawer.branch}
                  ...
                  <div className="flex justify-between items-center">
                    <span className="font-medium capitalize flex items-center gap-2">
                      ...
                      {order.fulfillment.type === 'pickup' ? t.orders.drawer.pickup : order.fulfillment.type === 'delivery' ? t.orders.drawer.delivery : t.orders.drawer.noShipping}
                    </span>
            ...
            {/* Financials & Items */}
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-gray-400 uppercase">{t.orders.drawer.itemsPricing}</h4>
              <div className="border border-gray-200 rounded-lg overflow-hidden">
                  ...
                  {(isEditing ? editForm.items : order.items)?.map((item, idx) => (
                    <div key={idx} className="p-3 bg-white flex justify-between items-center">
                      <div>
                        ...
                        <div className="text-xs text-gray-500">{item?.quantity} x {item?.price} {t.common.currencySar || 'SAR'}</div>
                      </div>
                      <div className="font-bold text-gray-900">{item?.total} {t.common.currencySar || 'SAR'}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="p-3 bg-gray-50 space-y-2 border-t border-gray-200">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-gray-600">{t.orders.drawer.discount}</span>
                    {isEditing ? (
                      <input 
                        type="number" 
                      ...
                      <span className="text-red-600">-{order.discount.toFixed(2)} {t.common.currencySar || 'SAR'}</span>
                    )}
                  </div>
                  ...
                  <div className="flex justify-between items-center text-sm">
                    ...
                    {isEditing ? (
                      ...
                      <span>{order.tax.toFixed(2)} {t.common.currencySar || 'SAR'} ({order.taxConfig.rate * 100}%)</span>
                    )}
                  </div>

                  {/* Total */}
                  <div className="flex justify-between font-bold text-lg text-gray-900 pt-2 border-t border-gray-200 mt-2">
                    <span>{t.inbox.orderPanel.total}</span>
                    <span>
                      {isEditing 
                       ? t.orders.drawer.recalculating
                       : `${order.total.toFixed(2)} ${t.common.currencySar || 'SAR'}`
                      }
                    </span>
                  </div>
                </div>
            ...
            {isEditing && (
              <div className="flex gap-2">
                <button onClick={() => setIsEditing(false)} className="flex-1 py-2 border rounded-lg hover:bg-gray-50">{t.common.cancel}</button>
                <button onClick={saveEdit} disabled={processing} className="flex-1 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 flex justify-center items-center gap-2">
                  <Save size={16} /> {t.orders.drawer.saveChanges}
                </button>
              </div>
            )}
            ...
            {!isEditing && canApprove && (
              <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm space-y-3">
                <h4 className="font-bold text-gray-800 flex items-center gap-2"><Shield size={16}/> {t.orders.drawer.approvalActions}</h4>
                {!showRejectInput ? (
                  <div className="flex gap-3">
                    ...
                    >
                      {t.orders.drawer.reject}
                    </button>
                  </div>
                ) : (
                  <div className="space-y-3">
                    <textarea 
                      placeholder={t.orders.drawer.rejectReason} 
                      className="w-full border border-gray-300 rounded-lg p-2 text-sm"
                      value={rejectReason}
                      onChange={e => setRejectReason(e.target.value)}
                    />
                    <div className="flex gap-2 justify-end">
                      <button onClick={() => setShowRejectInput(false)} className="text-gray-500 text-sm">{t.common.cancel}</button>
                      <button onClick={handleReject} disabled={!rejectReason} className="bg-red-600 text-white px-4 py-1.5 rounded text-sm font-medium">{t.orders.drawer.confirmReject}</button>
                    </div>
                  </div>
                )}
              </div>
            ...
            {/* History Log */}
            <div className="space-y-3">
              <h4 className="text-xs font-bold text-gray-400 uppercase">{t.orders.drawer.auditLog}</h4>
              <div className="space-y-4 ltr:pl-4 rtl:pr-4 border-l-2 rtl:border-l-0 rtl:border-r-2 border-gray-100">
            ...
            {!isEditing && (
              <div className="pt-4 border-t border-gray-200 space-y-3">
                {(order.invoice || order.status === 'completed') && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-blue-900">{t.orders.drawer.invoice}</h3>
                        <p className="text-xs text-blue-700">{order.invoice ? `#${order.invoice.number}` : t.orders.drawer.readyGenerate}</p>
                      </div>
                    </div>
                    {order.invoice ? (
                      ...
                      <button onClick={handleGenerateInvoice} disabled={generatingInvoice} className="flex items-center gap-1 text-sm font-bold text-blue-700 hover:underline disabled:opacity-50">
                        <RefreshCw size={16} className={generatingInvoice ? 'animate-spin' : ''} /> {t.orders.drawer.generate}
                      </button>
                    )}
                  </div>
                )}
                ...
                {order.paymentLink && order.status === 'pending-payment' && (
                  ...
                  >
                    <ExternalLink size={18} /> {t.orders.drawer.paymentLink}
                  </a>
                )}
              </div>
            )}
