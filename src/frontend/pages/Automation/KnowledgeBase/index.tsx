  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t.automation.knowledgeBase.title}</h1>
          <p className="text-gray-500 mt-1">{t.automation.knowledgeBase.desc}</p>
        </div>
        <button 
          onClick={handleAdd}
        ...
        >
          ...
          {t.automation.knowledgeBase.create}
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-gray-900">{currentArticle.id ? t.automation.knowledgeBase.editArticle : t.automation.knowledgeBase.newArticle}</h2>
              <button onClick={() => setIsEditing(false)} className="text-gray-400 hover:text-gray-600"><X size={24} /></button>
            </div>
            
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t.automation.knowledgeBase.table.category}</label>
                <input 
                  type="text" 
                  value={currentArticle.category || ''}
                  onChange={(e) => setCurrentArticle({ ...currentArticle, category: e.target.value })}
                  className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-green-500"
                  placeholder={t.automation.knowledgeBase.placeholders.category}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t.automation.knowledgeBase.table.question}</label>
                <input 
                  type="text" 
                  value={currentArticle.question || ''}
                  onChange={(e) => setCurrentArticle({ ...currentArticle, question: e.target.value })}
                  className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-green-500"
                  placeholder={t.automation.knowledgeBase.placeholders.question}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t.automation.knowledgeBase.table.answer}</label>
                <textarea 
                  value={currentArticle.answer || ''}
                  onChange={(e) => setCurrentArticle({ ...currentArticle, answer: e.target.value })}
                  rows={4}
                  className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-green-500"
                  placeholder={t.automation.knowledgeBase.placeholders.answer}
                />
              </div>
