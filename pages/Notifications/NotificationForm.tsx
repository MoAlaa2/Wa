...
const NotificationForm = () => {
  ...
  useEffect(() => {
    const init = async () => {
       ...
       const count = await whatsappService.getContactsCount();
  ...
  const handleSave = async (launch: boolean = false) => {
    ...
    const campaignToSave = {
      ...
      stats: { ...formData.stats, total: contactCount } 
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">{id ? t.common.edit : t.notifications.create}</h1>
        <p className="text-gray-500">{t.notifications.wizard.steps[STEPS[currentStep] as keyof typeof t.notifications.wizard.steps]}</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 md:p-8 min-h-[400px]">
        ...
        {currentStep === 0 && (
          <div className="space-y-6 max-w-lg mx-auto">
             <div className="text-center mb-8">
               ...
               <h3 className="text-lg font-medium">{t.notifications.wizard.steps.basic}</h3>
             </div>
             
             <div>
               <label className="block text-sm font-medium text-gray-700 mb-1">{t.notifications.wizard.basic.labelTitle}</label>
               <input 
                 type="text" 
                 value={formData.title} 
                 onChange={e => setFormData({...formData, title: e.target.value})}
                 className="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-500"
                 placeholder={t.notifications.wizard.basic.placeholderTitle}
               />
             </div>

             <div>
               <label className="block text-sm font-medium text-gray-700 mb-2">{t.notifications.wizard.basic.labelType}</label>
               <div className="grid grid-cols-2 gap-4">
                 ...
                 >
                   <div className="font-bold">{t.notifications.wizard.basic.types.broadcast}</div>
                 ...
                 >
                   <div className="font-bold">{t.notifications.wizard.basic.types.transactional}</div>
        ...
        {currentStep === 1 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             <div>
               <h3 className="font-medium text-gray-900 mb-4 flex items-center gap-2"><FileText size={18}/> {t.notifications.wizard.template.select}</h3>
             ...
             <div className="bg-gray-100 rounded-xl p-4 flex items-center justify-center">
                <div className="bg-white p-4 rounded-lg shadow-sm w-full max-w-xs">
                   <h4 className="text-xs font-bold text-gray-400 uppercase mb-2">{t.notifications.wizard.template.preview}</h4>
                   {formData.templateId ? (
        ...
        {currentStep === 2 && (
          <div className="max-w-lg mx-auto text-center space-y-6">
             <div className="mb-8">
               ...
               <h3 className="text-lg font-medium">{t.notifications.wizard.steps.recipients}</h3>
             </div>
             ...
             <div className="space-y-4 text-left">
               <label className="block p-4 border rounded-xl cursor-pointer hover:bg-gray-50 border-gray-200">
                  ...
                  <span className="font-medium">All Contacts ({contactCount})</span>
               </label>
               <label className="block p-4 border rounded-xl cursor-pointer hover:bg-gray-50 border-gray-200">
                  <input type="radio" name="list" className="mr-3" />
                  <span className="font-medium">VIP Customers (5)</span>
               </label>
               <div className="p-4 border border-dashed border-gray-300 rounded-xl bg-gray-50 opacity-60">
                  <div className="flex justify-between items-center">
                    <span className="font-medium text-gray-500">{t.notifications.wizard.recipients.smartSegments}</span>
                    <span className="text-xs bg-gray-200 px-2 py-1 rounded">{t.notifications.wizard.recipients.comingSoon}</span>
                  </div>
               </div>
             </div>
          </div>
        ...
        {currentStep === 3 && (
           <div className="max-w-lg mx-auto space-y-6">
              <div className="text-center mb-8">
               ...
               <h3 className="text-lg font-medium">{t.notifications.wizard.steps.advanced}</h3>
             </div>

             <div className="bg-gray-50 p-4 rounded-xl space-y-4 border border-gray-200">
                <label className="flex items-center justify-between cursor-pointer">
                  <span className="text-sm font-medium text-gray-700">{t.notifications.wizard.advanced.retry}</span>
                  <input 
                    type="checkbox" 
                ...
                <label className="flex items-center justify-between cursor-pointer">
                  <span className="text-sm font-medium text-gray-700">{t.notifications.wizard.advanced.emailReport}</span>
                  <input 
                    type="checkbox" 
                ...
                <div>
                   <span className="block text-sm font-medium text-gray-700 mb-1">{t.notifications.wizard.advanced.throttling}</span>
                   <input 
                     type="number" 
      ...
      <div className="flex justify-between mt-6">
        ...
        >
          ...
          {currentStep > 0 ? t.notifications.wizard.actions.back : t.common.cancel}
        </button>
        ...
        <div className="flex gap-3">
          {currentStep === STEPS.length - 1 && (
            ...
            >
              ...
              {t.notifications.wizard.actions.saveDraft}
            </button>
          )}
