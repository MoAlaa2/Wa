      ...
      <div className={`fixed inset-y-0 ${dir === 'rtl' ? 'left-0' : 'right-0'} max-w-full flex`}>
        <div className={`w-screen max-w-md transform transition ease-in-out duration-300 ${isOpen ? 'translate-x-0' : (dir === 'rtl' ? '-translate-x-full' : 'translate-x-full')} bg-white shadow-xl flex flex-col`}>
          ...
          <div className="flex items-center justify-between px-4 py-6 bg-gray-50 border-b border-gray-200">
            <h2 className="text-lg font-medium text-gray-900">{t.pages.templates.drawer.details}</h2>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8">
            ...
            <div>
              <div className="flex items-center space-x-2 rtl:space-x-reverse mb-4">
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium 
                  ...
                  {t.pages.templates.status[template.status]}
                </span>
                ...
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
                  <div className="flex">
                    ...
                    <div className="ml-3 rtl:ml-0 rtl:mr-3">
                      <h3 className="text-sm font-medium text-red-800">{t.pages.templates.drawer.rejectionReason}</h3>
            ...
            {/* Preview Section */}
            <div>
              <h3 className="text-sm font-medium text-gray-900 mb-3">{t.pages.templates.drawer.preview}</h3>
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-gray-100 pt-6">
              <div>
                <label className="text-xs text-gray-500 uppercase">{t.pages.templates.table.category}</label>
                <p className="text-sm font-medium text-gray-900">{template.category}</p>
              </div>
              <div>
                <label className="text-xs text-gray-500 uppercase">{t.pages.templates.table.language}</label>
            ...
            <div>
              <h3 className="text-sm font-medium text-gray-900 mb-3">{t.pages.templates.drawer.variables}</h3>
                ...
                {bodyComponent?.text?.match(/{{[0-9]+}}/g) ? (
                  bodyComponent.text.match(/{{[0-9]+}}/g)?.map((variable, idx) => (
                    <div key={idx} className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-gray-400">{t.pages.templates.drawer.variablePrefix} {idx + 1}</span>
                    </div>
                  ))
                ) : (
                  <span className="text-sm text-gray-400">{t.pages.templates.drawer.noVariables}</span>
                )}
              </div>
            </div>
