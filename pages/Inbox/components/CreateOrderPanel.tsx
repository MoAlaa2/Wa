  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      if (order.paymentLink) {
        ...
        alert(`Order #${order.orderNumber} submitted for approval.`);
        onClose();
      }
    } catch (e) {
    ...
    <div className="w-full h-full flex flex-col bg-gray-50 border-l border-gray-200">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
          <label className="text-xs font-bold text-gray-400 uppercase mb-2 block">Customer</label>
        ...
        {isTrainingMode && (
          <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg flex items-start gap-2 text-xs text-orange-800">
            ...
            <p>Training Mode: Custom items and price overrides are disabled.</p>
          </div>
        )}
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-3">
          <div className="flex justify-between items-center">
             ...
             {!isTrainingMode && (
               ...
               >
                 {isCustomMode ? 'Search Catalog' : '+ Custom Item'}
               </button>
             )}
          </div>
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-20 max-h-60 overflow-y-auto">
                  {products.map(p => (
                    ...
                    >
                      ...
                      <div className="font-bold text-green-600">{p.price} SAR</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            {items.map((item, idx) => (
              <div key={idx} className="flex justify-between items-center p-2 bg-gray-50 rounded-lg border border-gray-100">
                <div className="flex-1 min-w-0 mr-2">
                  ...
                  <div className="text-xs text-gray-500">{item.price} SAR x {item.quantity}</div>
